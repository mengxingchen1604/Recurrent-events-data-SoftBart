fit_RecSBART_application <- function(X_train,
                                    X_test,
                                    recurrent_train,
                                    recurrent_test,
                                    terminal_train,
  ...
  cumulative_mean <- array(0, dim=c(nrow(X_test),length(grid)))
  start.time <- Sys.time()
  ...
  for (iter in 1:num_iter) {
    ...
    shape_W_prior <- (diversitree::mcmc(
      ...
      prior = function(shape_W_prior)
        dgamma(shape_W_prior, shape = 40, rate = 0.1, log = TRUE),
    ...
    if (iter > num_burn && ((iter - num_burn - 1) %% num_thin == 0)) {
      ...
      cumulative_intensity_estimate <- matrix(NA, nrow = nrow(X_test), ncol = length(grid))
      
      for (i in 1:nrow(X_test)) {
        XT <- matrix(rep(X_test[i, ], length(grid)), ncol = p, nrow = length(grid), byrow = TRUE)
        phi <- pnorm(as.numeric(my_forest$do_predict(cbind(grid, XT))))
        integral <- cumsum(phi) * 0.05
        cumulative_intensity_estimate[i, ] <- 2 * lambda * integral
      }
      
      # Update running (posterior) mean using online average
      for (i in 1:nrow(X_test)) {
        cumulative_mean[i, ] <- (1 - frac) * cumulative_mean[i, ] + frac * cumulative_intensity_estimate[i, ]
      }
  ...
  return(list(cumulative_mean = cumulative_mean))
}
